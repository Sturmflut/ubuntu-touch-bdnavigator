...
import ubuntu-touch-bdnavigator
...
class BaseTestCase(AutopilotTestCase):
    ...
    click_package = '{0}.{1}'.format('ubuntu-touch-bdnavigator', 'sturmflut')

    def setUp(self):
        super(BaseTestCase, self).setUp()
        self.launcher, self.test_type = self.get_launcher_and_type()
        self.app = ubuntu-touch-bdnavigator.TouchApp(self.launcher(), self.test_type)
